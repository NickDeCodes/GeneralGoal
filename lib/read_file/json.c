/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 五 10/ 2 19:48:22 2020
 ************************************************************************/
...
#if (JSON_VERSION_MAJOR != 1) || (JSON_VERSION_MINOR != 7) || (JSON_VERSION_PATCH != 4)
    #error cJSON.h and cJSON.c have different versions. Make sure that both have the same.
#endif

JSON_PUBLIC(const char*) JSON_Version(void) {
    static char version[15];
    sprintf(version, "%i.%i.%i", JSON_VERSION_MAJOR, JSON_VERSION_MINOR, JSON_VERSION_PATCH);
...
static JSON *JSON_New_Item(const internal_hooks * const hooks) {
    JSON* node = (JSON*)hooks->allocate(sizeof(JSON));
    if (node) {
...
JSON_PUBLIC(void) JSON_Delete(JSON *item) {
    JSON *next = NULL;
    while (item != NULL) {
        next = item->next;
...
static JSON_bool parse_number(JSON *const item, parse_buffer *const input_buffer) {
    double number = 0;
...
static JSON_bool print_number(const JSON *const item, printbuffer *const output_buffer) {
...
static JSON_bool parse_string(JSON * const item, parse_buffer * const input_buffer) {
...
static JSON_bool print_string(const JSON *const item, printbuffer *const p) {
    return print_string_ptr((unsigned char*)item->valuestring, p);
}

/* Predeclare these prototypes. */
static JSON_bool parse_value(JSON *const item, parse_buffer *const input_buffer);
static JSON_bool print_value(const JSON *const item, printbuffer *const output_buffer);
static JSON_bool parse_array(JSON *const item, parse_buffer *const input_buffer);
static JSON_bool print_array(const JSON *const item, printbuffer *const output_buffer);
static JSON_bool parse_object(JSON *const item, parse_buffer *const input_buffer);
static JSON_bool print_object(const JSON *const item, printbuffer *const output_buffer);
...
JSON_PUBLIC(JSON *) cJSON_ParseWithOpts(const char *value, const char **return_parse_end, JSON_bool require_null_terminated) {
...
static unsigned char *print(const JSON * const item, JSON_bool format, const internal_hooks * const hooks) {
...
JSON_PUBLIC(JSON_bool) JSON_PrintPreallocated(JSON *item, char *buf, const int len, const JSON_bool fmt) {
    printbuffer p = { 0, 0, 0, 0, 0, 0, { 0, 0, 0 } };
...
static JSON_bool parse_array(JSON * const item, parse_buffer * const input_buffer) {
    ...
    do {
        /* allocate next item */
        JSON *new_item = JSON_New_Item(&(input_buffer->hooks));
        if (new_item == NULL) {
            goto fail; /* allocation failure */
        }
...
static JSON_bool print_array(const JSON * const item, printbuffer * const output_buffer) {
    unsigned char *output_pointer = NULL;
    size_t length = 0;
    JSON *current_element = item->child;
...
static JSON* get_array_item(const JSON *array, size_t index) {
    JSON *current_child = NULL;
...
static JSON *get_object_item(const JSON * const object, const char * const name, const JSON_bool case_sensitive) {
    JSON *current_element = NULL;
...
static JSON *create_reference(const JSON *item, const internal_hooks * const hooks) {
    JSON *reference = NULL;
    if (item == NULL) {
        return NULL;
    }
...
static JSON_bool add_item_to_array(JSON *array, JSON *item) {
    JSON *child = NULL;
...
static JSON_bool add_item_to_object(JSON * const object, const char * const string, JSON * const item, const internal_hooks * const hooks, const JSON_bool constant_key) {
    char *new_key = NULL;
...
{
    JSON *true_item = JSON_CreateTrue();
...
{
    JSON *false_item = JSON_CreateFalse();
...
{
    JSON *bool_item = JSON_CreateBool(boolean);
...
{
    JSON *number_item = JSON_CreateNumber(number);
...
{
    JSON *string_item = JSON_CreateString(string);
...
{
    JSON *raw_item = JSON_CreateRaw(raw);
...
{
    JSON *object_item = JSON_CreateObject();
...
JSON_PUBLIC(JSON *) JSON_DetachItemFromArray(JSON *array, int which)
{
    ...
    return JSON_DetachItemViaPointer(array, get_array_item(array, (size_t)which));
}
...
{
    JSON *to_detach = JSON_GetObjectItem(object, string);
...
{
    JSON *to_detach = JSON_GetObjectItemCaseSensitive(object, string);
...
JSON_PUBLIC(void) JSON_InsertItemInArray(JSON *array, int which, JSON *newitem)
{
    JSON *after_inserted = NULL;
...
JSON_PUBLIC(void) JSON_ReplaceItemInArray(JSON *array, int which, JSON *newitem)
{
    ...
    JSON_ReplaceItemViaPointer(array, get_array_item(array, (size_t)which), newitem);
}

static JSON_bool replace_item_in_object(JSON *object, const char *string, JSON *replacement, JSON_bool case_sensitive)
{
    ...
    JSON_ReplaceItemViaPointer(object, get_object_item(object, string, case_sensitive), replacement);
...
{
    JSON *item = JSON_New_Item(&global_hooks);
    if(item)
    {
        item->type = JSON_NULL;
    }
...
JSON_PUBLIC(JSON *) JSON_CreateBool(JSON_bool b)
{
    JSON *item = JSON_New_Item(&global_hooks);
    if(item)
    {
...
JSON_PUBLIC(JSON *) JSON_CreateObjectReference(const JSON *child)
{
    ...
    if (item != NULL) {
        ...
        item->child = (JSON*)cast_away_const(child);
    }
...
JSON_PUBLIC(JSON *) JSON_Duplicate(const JSON *item, JSON_bool recurse)
{
    ...
    if (item->string)
    {
        newitem->string = (item->type&JSON_StringIsConst) ? item->string : (char*)JSON_strdup((unsigned char*)item->string, &global_hooks);
        if (!newitem->string)
        {
            goto fail;
        }
...
{
    if ((a == NULL) || (b == NULL) || ((a->type & 0xFF) != (b->type & 0xFF)) || JSON_IsInvalid(a))
    {
        return false;
    }
    ...
    switch (a->type & 0xFF)
    {
        ...
        case JSON_Array:
        {
            JSON *a_element = a->child;
            JSON *b_element = b->child;
